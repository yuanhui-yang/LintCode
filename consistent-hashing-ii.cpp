...
public:
    ...
    vector<int> addMachine(int machine_id) {
        ...
        for (int i = 0; i < this->k; ++i) {
            int hashcode = f1();
            this->table[hashcode] = machine_id;
    ...
    int getMachineIdByHashCode(int hashcode) {
        // write your code here
        map<int, int>::iterator it = this->table.lower_bound(hashcode);
        if (this->table.empty()) {
            return -1;
        }
        else if (it != this->table.end()) {
            return it->second;
        }
        else {
            return this->table.begin()->second;
        }
    }
